...
class UserController extends Controller
{
    ...
    public function actionUpdate($id)
    {
        ...
        if (isset($_POST['User'])) {
            $model->attributes = $_POST['User'];
            if (!empty($model->password_baru)) {
                $model->password = $model->hashPassword($model->password_baru);
            }
            if ($model->save()) {
                Yii::app()->user->setFlash('update', 'Data user berhasil disimpan.');
                $this->refresh();
            }
        }
    ...
    {
        $dataProvider = Crawler::getDataProvider(array('site'), false);
        foreach ($dataProvider as $data) {
            $num[] = count(Crawler::getInternalActions($data['controller'] . 'Controller'));
        }
        rsort($num);
        $model = new HakAksesForm;
        if(isset($_POST['HakAksesForm'])){
            $model->attributes = $_POST['HakAksesForm'];
            $user = $this->loadModel($_GET['id']);
            $user->hak_akses = CJSON::encode($model->access);
            if($user->update('hak_akses')) {
